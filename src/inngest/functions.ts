...
export const codeAgentFunction = inngest.createFunction(
  ...
  async ({ event, step }) => {
    try {
      ...
      await step.run("ensure-sandbox-base-files", async () => {
      ...
      if (normalizedFileEntries.length > 0) {
        await step.run("normalize-client-component-files", async () => {
          const sandbox = await getSandbox(sandboxId);

          for (const [path, content] of normalizedFileEntries) {
            await sandbox.files.write(path, content);
          }
        });

        result.state.data.files = normalizedFiles;
      }
